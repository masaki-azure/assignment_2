...
- **App Settings への保存はスコープ外**:
  `sensitive = true` により、Terraform への入力は実行時の変数渡しに変更済み。
  ただし出力側（Azure に書き込まれた後）は App Service の構成に平文のまま保存され、Portal や CLI から読み取れる状態が残る。Key Vault 参照への切り替えは今回のスコープ外の TODO とする。
